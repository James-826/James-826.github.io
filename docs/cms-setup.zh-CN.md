...
6. 默认关闭 **保存为草稿**，保存后会直接发布；需要先暂存时再打开它。

保存会向 `master` 提交 Markdown 和图片，并自动触发 GitHub Actions。
如果开启了 **保存为草稿**，文件仍会提交到仓库，但生产构建不会显示它，你可以在 CMS 中继续修改。

### 3. 发布和暂存

需要发布暂存文章时，关闭 **保存为草稿** 后再次保存；需要下线但保留内容时，重新打开它并保存。
